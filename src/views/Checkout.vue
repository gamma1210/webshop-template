        ...
        <div class="md:col-span-3">
          ...
          <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input type="text" v-model="phone" placeholder="Phone number" class="input-field" />
              <input type="text" v-model="city" placeholder="City" class="input-field" />
              <input type="text" v-model="postCode" placeholder="Post Code" class="input-field" />
            </div>
            <input type="text" v-model="address" placeholder="Address" class="input-field w-full mt-4" />
          </div>
        ...
        <div class="md:col-span-2 bg-white p-6 rounded-lg shadow-md">
          ...
          <div class="mt-6 border-t pt-4 space-y-2">
            ...
            <p class="flex justify-between font-bold text-lg">
              ...
              <span>
                Rp {{ formatPrice((product?.curr_price * quantity || 0) + shippingCost) }}
              </span>
            </p>
          </div>

          <!-- Continue Button as Form Submit -->
          <button type="submit"
    ...
    <div v-if="showPopup" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div class="bg-white p-6 rounded-lg shadow-lg text-center max-w-sm mx-auto">
        ...
        <button @click="continueShopping"
          class="mt-6 px-4 py-2 bg-black text-white rounded hover:bg-stone-300 transition">
          Continue Shopping
        </button>
      </div>
    </div>
...
async function handleContinue() {
  ...
  const raw = JSON.stringify({
    ...
    paymentMethod: "QRIS",
    amount: {
      value: product.value.curr_price,
      currency: "IDR"
    },
  ...
  // Uncomment below when ready to use the API:
  try {
    const response = await fetch(import.meta.env.VITE_API_ENDPOINT + "/create-bill", requestOptions);
    const result = await response.json();
    localStorage.setItem("orderBillResponse", result.qrUrl);
    openPaymentWindow();
  } catch (error) {
    alert("Error in creating bill. Please try again later.");
